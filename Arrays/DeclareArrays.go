...
func beatlesArrayExample() {
	...
	fmt.Println("The name of the third band member in the beatles array is", beatles[2])
	fmt.Println("Length of beatles: ", len(beatles))

	// In Go arrays are values. When we assign one array to another, all
	// the elements from the array on the right hand side are copied over
	// to the array on the left hand side.
	...
	// the memory address of the tow arrays is not a criteria for the comparison.
...
func u2ArrayExample() {
	...
	// to initialize the array, we get the job done in 1 line of coding using an array
	// literal value
	u2 := [4]string{"Bono", "Edge", "Adam", "Larray"}
...
func main() {
	...
	// Note: If we do not initialize a value for the arrau, they will default
	// to the zero value of the type. In the case of integers, we expect the
	// zero value to be 0
	var myArray [5]int
	fmt.Printf("Contents of myArray: %v\n\n", myArray)

	// Arrays are passed by value to functions, meaning a copy of the Array
	// is passed and not a refrence (pointer) to the array
